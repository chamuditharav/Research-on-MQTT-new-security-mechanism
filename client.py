...
class MQTTS_CLIENT(Thread):
    ...
    def __init__(self, clientID, client_nick, broker_ip, broker_port=1883):
    ...
    def init_rolling_code_thread(self,start,step,end):
      if(self.key_ex_status == False):
        ...
        try:
            ...
            dataFrame = encrypt(dataFrame, self.public_key) # rolling code data is encrypted b4 transmitting
            self.client.publish(self.broker_tx,f"INIT-ROLL:{self.client_nick}:{dataFrame.hex()}") #sending in hex format
            #print(f"INIT-ROLL:{dataFrame}")
            #print(dataFrame)
        except:
          print("[-] Roling key set faild")

    def init_rolling_code(self,start,step,end): #rolling code start, increment step, ending val
    ...
    def key_ex_init(self,public_key,private_key):
      try:
        ...
        pubKhex = public_key._save_pkcs1_pem().hex() #Sending the pub key in hex format.
    ...
    def sendEncMSG(self,msg,dest):
      if(self.public_key == ""):
        ...
        dataFrame = f"{self.client_id}:{self.client_nick}:{dest}:{msg}"
        cipher_msg = encrypt(dataFrame,self.public_key)                 # encrypted message with destination
        cipher_count = encrypt(str(self.key_count), self.public_key)    # encrypted current rolling code value
...
client = MQTTS_CLIENT("001","client1", "192.168.1.4", 1883)


client.sendUnencMSG("Hello from client 1","client2")
client.loop()
#client.showConfig()
client.key_ex()
sleep(1)
client.init_rolling_code(10,1,15)
sleep(5)

while(True):
  #client.init_rolling_code(100,10,1000)

  client.sendEncMSG("Encrypted Hello from Client 1","client2")
  sleep(5)
  client.sendUnencMSG("Unencrypted Hello from client 1","client2")
